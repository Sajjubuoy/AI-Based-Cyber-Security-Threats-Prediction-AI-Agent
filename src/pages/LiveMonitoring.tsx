    ...
    <div className="min-h-screen bg-gradient-background p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Real-time threat detection and monitoring</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleToggleStream} variant="outline" className="gap-2">
            {isStreaming ? (
              <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <MetricCard title="Events/Second" value={eventsPerSecond.toLocaleString()} icon={Zap} description="Current throughput" />
        <MetricCard title="Total Events" value={totalEvents.toLocaleString()} icon={Activity} description="Since session start" />
        <MetricCard title="Active Threats" value={activeThreats} icon={AlertCircle} description="Currently monitoring" />
        <MetricCard title="Blocked" value={blockedThreats} icon={Radio} description="Real-time prevention" />
      </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  <p className="text-xl font-bold mt-1">{trafficData[trafficData.length - 1]?.incoming || 0} MB/s</p>
                </div>
                <div className="p-3 bg-muted rounded-lg">
                  <p className="text-sm text-muted-foreground">Outgoing Traffic</p>
                  <p className="text-xl font-bold mt-1">{trafficData[trafficData.length - 1]?.outgoing || 0} MB/s</p>
                </div>
                <div className="p-3 bg-destructive/10 border border-destructive rounded-lg">
                  <p className="text-sm text-muted-foreground">Threat Events</p>
                  <p className="text-xl font-bold mt-1 text-destructive">{trafficData[trafficData.length - 1]?.threats || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary" className="bg-success/10 text-success">Stable</Badge>
                </div>
                <div className="flex items-center justify-between">
