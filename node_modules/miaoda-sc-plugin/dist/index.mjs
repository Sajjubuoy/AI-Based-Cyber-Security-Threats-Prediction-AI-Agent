...
function monitorPlugin(options = {}) {
  const {
    ...
    consoleLevels = ["log", "info", "warn", "error", "debug", "assert"],
      ...
      handler(html) {
        ...
        if (enableHttpTracing) {
          const originsJson = JSON.stringify(
            tracingOrigins.map(
              (origin) => origin instanceof RegExp ? origin.source : origin
            )
          );
          integrations.push(`
          Sentry.browserTracingIntegration({
            traceFetch: true,
            traceXHR: true,
            tracingOrigins: ${originsJson}
          })
        `);
        }
        if (enableConsoleCapture && consoleLevels.length > 0) {
          const levelsJson = JSON.stringify(consoleLevels);
          integrations.push(`
        ...
        return {
          ...
          tags: [
            // 1. 同步加载 Sentry SDK（阻塞式加载，确保最先执行）
            {
              ...
              attrs: {
                id: "sentry-sdk",
                src: "https://agentos-sandbox.bj.bcebos.com/vite-sandbox-deps/browser.sentry.min.js",
              ...
              (function() {
                ...
                try {
                  ...
                  console.log('[Monitor Plugin] Sentry initialized successfully', {
                    dsn: '${sentryDsn}',
                    environment: '${environment}',
                    enableHttpTracing: ${enableHttpTracing},
